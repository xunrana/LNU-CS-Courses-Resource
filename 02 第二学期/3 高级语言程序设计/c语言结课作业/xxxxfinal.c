...
void show_menu(void);                     //显示主菜单并提示用户选择身份。 

void show_1(void);                        // 显示管理员菜单选项，包括添加、删除和查看学生账户。
void add_st_1_1(St **head,char *num);     //向链表中添加新的学生账号。
void delete_st_1_2(St **head,char* num);  //在链表中删除特定的学生账号。 
void show_1_3(St *head);                  //显示链表中的所有学生账号。 

 
void show_2(void);                        //显示教师菜单选项，包括查询、输入和修改[姓名已脱敏]。
void show_2_1(void);                      //显示按班级结果查看的选项，并提示用户选择特定的专业。
St *sort_class(St *head);                 //根据总成绩对学生记录进行排序，并分配班级和专业排名。
St *sort_sum(St *head);                   //根据总成绩对学生记录进行排序，并分配年级排名。
void sort_st(St *head,char *major);       //根据特定的专业对学生记录进行排序，并分配班级排名。
St *sort_major(St *head);                  //将特定专业的学生创建一个新链表，显示完后全部free。
void show_major_2_1_1(St *head,char *major);//显示特定专业的学生记录。
void show_st(St *head);                    //显示所有学生记录。
void show_2_2_1(void);                    //显示提示，输入学生的成绩。
void add_st(St **head,char *num);         //添加学生的成绩。
void show_2_3(void);                      //显示修改[姓名已脱敏]的选项。
void change_st(St **head,char* num);      //修改学生的成绩。
 

void show_3(void);                        //显示学生菜单选项，用于查询自己的成绩。
void show_3_1(void);                      //显示查看特定科目成绩的选项。
St find(St *head,char* num);              //根据[姓名已脱敏]查找特定学生的记录。
int main()
{
    ...
    do
    {
        ...
        while(a==2)
        {
            ...
            if(b==3)
            {
				...
				{
					f=0;
					puts("请输入你要录入或修改的学生账号:(输入quit返回上一步)");
				    scanf("%s",num1);
				    if(strcmp(num1,"quit")==0) {f1=1;break;}
				    for(p=head;p;p=p->next) if(strcmp(p->num,num1)==0) {f=1;break;}
				    if(f==1) break;
...
void show_2(void)
{
    ...
    puts("=======================================");
    puts("1:查询成绩");
    puts("2:录入[姓名已脱敏]        (未录入该学生账号)");
    puts("3:录入或修改[姓名已脱敏]  (已录入该学生账号)");
    puts("4:返回上一步");
    puts("请选择你想使用的功能:");
}
void show_2_1(void)
