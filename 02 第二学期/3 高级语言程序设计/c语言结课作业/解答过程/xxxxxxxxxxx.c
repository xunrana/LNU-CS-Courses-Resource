...
void add_st_2_2_2(St **tail);
void change_st_2_2_3(St *head,char *num);
void show_2_3(void); 
        ...
        while(a==1)
        {
            ...
            if(b==2)
            {
                ...
                while(1)
				{
				    puts("请输入你要删除的学生账号:");
                    scanf("%s",num1);
                    for(p=head;p;p=p->next) if(strcmp(p->num,num1)==0) break;
					puts("[姓名已脱敏]输入错误,请重新输入。");
				}
