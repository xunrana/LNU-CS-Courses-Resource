...
#define RATE4   0.34025       // rate for over 7[[姓名已脱敏]已脱敏] kwh       
#define BREAK1  360.0         // first breakpoint for rates  
#define BREAK2  468.0         // second breakpoint for rates 
#define BREAK3  7[[姓名已脱敏]已脱敏].0         // third breakpoint for rates
#define BASE1   (RATE1 * BREAK1)
// cost for 360 kwh            
#define BASE2  (BASE1 + (RATE2 * (BREAK2 - BREAK1)))
// cost for 468 kwh
#define BASE3   (BASE1 + BASE2 + (RATE3 *(BREAK3 - BREAK2)))
//cost for 7[[姓名已脱敏]已脱敏] kwh
int main(void)
{
    ...
    else if (kwh <= BREAK3)   // kwh betweent 468 and 7[[姓名已脱敏]已脱敏]
