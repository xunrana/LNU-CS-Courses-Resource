...
void print_first()
{
    ...
    for(int i=1;i<=num;i++)
    {
        ...
        for(int j=0;j<first[i].size();j++) cout<<getchar1[first[i][j]-'0']<<" ";
        cout<<endl;
    }
    cout<<endl;
}
	...
	for(int i=0;i<proce.size();i++)   //扫所有产生式
   	{
   		if(proce[i][1]=='@')     //直接推出空字的，特判下（follow集=产生式左边的vn中元素填）
        {
            ...
            for(int k=0;k<flw.size();k++) table[getnum[proce[i][0]]][flw[k]-'0']=i;    
        }
       	string temps=first[getnum[proce[i][1]]];
       	for(int j=0;j<temps.size();j++)               //考察first集
       	{
       		if(temps[j]!=('0'+numvt)) table[getnum[proce[i][0]]][temps[j]-'0']=i;
	        else                                     //有空字的，考察follw集
	        {
...
{
    cout<<"预测分析表如下："<<endl;
	for(int i=0;i<numvt;i++) cout<<'\t'<<getchar1[i];
    cout<<endl;
   	for(int i=numvt+1;i<=num;i++)
    {
       cout<<getchar1[i];
       for(int j=0;j<numvt;j++) cout<<'\t'<<get_proce(table[i][j]);
       cout<<endl;
    }
    cout<<endl;
}
void print_follow()
{
    ...
    for(int i=numvt+1;i<=num;i++)
    {
    	cout<<"follow ["<<getchar1[i]<<"]: ";
        for(int j=0;j<follow[i].size();j++) cout<<getchar1[follow[i][j]-'0']<<" ";
        cout<<endl;
    }
    cout<<endl;
}
...
int main()
{
    ...
    print_table();
   	cout<<"请输入字："<<endl;
   	cin>>word;
    if(analyze()) cout<<"succeed!该字有效，所用产生式如上。"<<endl;
